...
class GrappelliTests(TestCase):
    ...
    def test_related_lookup(self):
        ...
        response = self.client.get(
            "{}?object_id=1&app_label={}&model_name={}".format(
                reverse("grp_related_lookup"), "grappelli_test", "category"
            )
        )
        self.assertEqual(response.status_code, 200)
        ...
        # wrong object_id
        response = self.client.get(
            "{}?object_id=10000&app_label={}&model_name={}".format(
                reverse("grp_related_lookup"), "grappelli_test", "category"
            )
        )
        self.assertEqual(response.status_code, 200)
        ...
        response = self.client.get(
            "{}?object_id={}&app_label={}&model_name={}".format(
                reverse("grp_related_lookup"), self.entry_superuser.id, "grappelli_test", "entry"
            )
        )
        self.assertEqual(response.status_code, 200)
        self.assertJSONEqual(
        ...
        response = self.client.get(
            "{}?object_id={}&app_label={}&model_name={}".format(
                reverse("grp_related_lookup"), self.entry_editor.id, "grappelli_test", "entry"
            )
        )
        self.assertEqual(response.status_code, 200)
        self.assertJSONEqual(
    ...
    def test_m2m_lookup(self):
        ...
        # ok (single)
        response = self.client.get(
            "{}?object_id=1&app_label={}&model_name={}".format(reverse("grp_m2m_lookup"), "grappelli_test", "category")
        )
        self.assertEqual(response.status_code, 200)
        self.assertJSONEqual(
            response.content.decode("utf-8"), [{"value": "1", "label": "Category No 0 (1)", "safe": False}]
        ...
        response = self.client.get(
            "{}?object_id=10000&app_label={}&model_name={}".format(
                reverse("grp_m2m_lookup"), "grappelli_test", "category"
            )
        )
        self.assertEqual(response.status_code, 200)
        self.assertJSONEqual(response.content.decode("utf-8"), [{"value": "10000", "label": "?", "safe": False}])
        ...
        # ok (multiple)
        response = self.client.get(
            "{}?object_id=1,2,3&app_label={}&model_name={}".format(
                reverse("grp_m2m_lookup"), "grappelli_test", "category"
            )
        )
        self.assertEqual(response.status_code, 200)
        ...
        response = self.client.get(
            "{}?object_id=1,10000,3&app_label={}&model_name={}".format(
                reverse("grp_m2m_lookup"), "grappelli_test", "category"
            )
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_autocomplete_lookup(self):
        ...
        response = self.client.get(
            "{}?term=XXXXXXXXXX&app_label={}&model_name={}".format(
                reverse("grp_autocomplete_lookup"), "grappelli_test", "category"
            )
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_switch_login(self):
        ...
        self.client.login(username="Superuser001", password="superuser001")
        response = self.client.get(reverse("admin:grappelli_test_category_changelist"))
        self.assertEqual(response.status_code, 200)

        # template tag (Editor001, Editor002)
        t = switch_user_dropdown(response.context)
        t_cmp_1 = f"/grappelli/switch/user/{self.editor_1.id}/?redirect=/admin/grappelli_test/category/"
        t_cmp_2 = "Editor001"
        t_cmp_3 = f"/grappelli/switch/user/{self.editor_2.id}/?redirect=/admin/grappelli_test/category/"
        t_cmp_4 = "Editor002"
        ...
        self.client.login(username="User001", password="user001")
        response = self.client.get(reverse("admin:grappelli_test_category_changelist"), follow=True)
    ...
    def test_switch_superuser001_superuser002(self):
        ...
        response = self.client.get(
            "{}?redirect={}".format(
                reverse("grp_switch_user", args=[target_user.id]), reverse("admin:grappelli_test_category_changelist")
            ),
            follow=True,
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_switch_superuser001_editor001(self):
        ...
        t = switch_user_dropdown(response.context)
        t_cmp_1 = f"/grappelli/switch/user/{self.superuser_1.id}/?redirect=/admin/grappelli_test/category/"
        t_cmp_2 = "Superuser001"
        t_cmp_3 = f"/grappelli/switch/user/{self.editor_1.id}/?redirect=/admin/grappelli_test/category/"
        t_cmp_4 = "Editor001"
        t_cmp_5 = f"/grappelli/switch/user/{self.editor_2.id}/?redirect=/admin/grappelli_test/category/"
        t_cmp_6 = "Editor002"
        self.assertIn(t_cmp_1, t)
        self.assertIn(t_cmp_2, t)
        ...
        response = self.client.get(
            "{}?redirect={}".format(
                reverse("grp_switch_user", args=[original_user.id]),
                reverse("admin:grappelli_test_category_changelist"),
            ),
            follow=True,
        )
        self.assertEqual(response.status_code, 200)
